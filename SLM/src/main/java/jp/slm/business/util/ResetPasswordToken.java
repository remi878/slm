...
public class ResetPasswordToken {
	...
	private static final String TOKEN_TYPE = "RESET_PASSWORD";
	...
	public static ResetPasswordToken getNewToken(long userId) {
		return new ResetPasswordToken(userId);
	}
	
	public static ResetPasswordToken readToken(String tokenStr) {
		ResetPasswordToken token = null;
		if (isResetPasswordToken(tokenStr)) {
			token = new ResetPasswordToken(tokenStr);
		}
		return token;
	}
	...
	private ResetPasswordToken(long userId) {
		this.userId = userId;
	...
	private ResetPasswordToken(String tokenStr) {
		if (StringUtils.isNotBlank(tokenStr)) {
			String decrypted = PasswordCrypter.decodeStr(tokenStr);
			if (StringUtils.isNotBlank(decrypted)) {
				String[] tokens = decrypted.split(TOKEN_SEP);
	...
	public String toString() {
		...
		if (this.valid) {
			...
			sb.append(this.userId).append(TOKEN_SEP).append(this.timeStamp).append(TOKEN_SEP).append(this.randomNumber).append(TOKEN_SEP)
					.append(TOKEN_TYPE);
